namespace Admin.Controllers
{
        ...
        public RoleAuthorizationController(IConfiguration configuration, IHttpContextAccessor accessor, IOptions<ApiSettings> ApiSettings, IOptions<MailSettings> MailSettings, IWebHostEnvironment env, ILogger<RoleAuthorizationController> logger) : base(configuration)
    {
        ...
        {
            return await ApiClient.Get_ApiValues(getHttpClient(), "api/RoleAuthorization/prefill_RoleAuthorization_roleauthorized?userrole=" + userrole + "&loginUserID=" + HttpContext.Session.GetString("MarketplacenewloginUserID"));
        ...
        public virtual async Task<string> Add_Role(AddRoleModel model)
        {
            ...
            try
            {
                ...
                if (HttpContext.Session.GetString("MarketplacenewloginUserID") != null)
                    model.rolesid = new Guid(HttpContext.Session.GetString("MarketplacenewloginUserID"));
                else
                    return "Session Expired";
